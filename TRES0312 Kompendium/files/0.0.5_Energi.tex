...
\section{Arbeid}
I forrige kapittel så vi at 
\[\frac{\vec{p}}{\Delta t}=\vec{F}\quad\text{som medfører}\quad \Delta\vec{p}=\vec{F}\Delta t.\]
Dette viser at kraft gjort over tid gir endring i bevegelsesmengde. For dere som kan å integrere, ser vi altså at \textit{integralet} av kraft over tid er bevegelsesmengde. Dette gir mening. Når en kraft virker på et legeme over tid, så endrer det bevegelsesmengden til legemet. Dette har faktisk et eget navn: \textit{impuls}.
I hverdagslivet bruker vi ordet \textit{arbeid} om noe vi gjør over tid. I fysikk er begrepet mer presist: arbeid utføres når en kraft forskyver et objekt. Det er ikke nok at kraften er til stede – den må faktisk bidra til bevegelsen. En person som holder en tung sekk stille over hodet, utfører for eksempel \emph{ikke} noe arbeid i fysisk forstand, selv om muskler spenner.
...
Legg merke til at \( \cos\theta \) avgjør hvor mye av kraften som «drar» i bevegelsesretningen. Når \( \theta = 0^\circ \) virker kraften rett frem og arbeidet er maksimalt. Når \( \theta = 90^\circ \) er kraften vinkelrett på bevegelsen, og \( W = 0 \) – typisk for en normalkraft eller sentripetalkraft. Virker kraften \emph{mot} bevegelsen (som friksjon), er \( \theta = 180^\circ \) og arbeidet negativt.
...
\begin{minipage}{0.45\textwidth}
    ...
    \captionof{figure}{Konstant kraft: \( W = F \cdot s \) er arealet av rektangelet under kurven.}
    \label{fig:image1}
\end{minipage}\hfill
\begin{minipage}{0.45\textwidth}
    \centering
    \includegraphics[width=\linewidth]{Images/Kapittel2:Kinematikk/Arbeidpåfjærgraf.png}
    \captionof{figure}{Lineært voksende kraft \( F = kx \): \( W = \tfrac{1}{2}kx^2 \) er arealet av trekanten under kurven.}
    \label{fig:image2}
\end{minipage}
