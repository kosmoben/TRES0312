...
\input{Oppgaver/k4_oppg03.tex}%
}
\oppgaver{Fritt fall}{k4oppg4}{%
\input{Oppgaver/k4_oppg04.tex}%
}
...
\input{Oppgaver/k4_oppg07.tex}%
}

\oppgaver{Bremselengde for bildekk}{k4oppg8}{%
\input{Oppgaver/k4_oppg08.tex}%
}
...
\begin{minipage}[t]{0.54\linewidth}
\vspace{0pt}
Som et første eksempel ser vi på en sleggekaster som svinger slegga rundt seg. Slegga er en metallkule festet til en wire, og sleggekasteren holder i håndtaket i den andre enden. Sett ovenfra går kula i en sirkelbane rundt sleggekasterens hånd. Snorkraften $\vec{S}$ fra wiren peker hele tiden inn mot sentrum — det er denne kraften som gir kula sentripetalakselerasjonen. (Vi forenkler ved å se bort fra tyngdekraften, og ser bare på kreftene i sirkelens plan.)
\end{minipage}%
\hfill
\begin{minipage}[t]{0.43\linewidth}
\vspace{0pt}
...
\begin{tikzpicture}[font=\small]
  ...
  \node[font=\scriptsize, below left=2pt] at (0,0) {hånd};
  % Wire
  ...
  \draw[-{Stealth[length=5pt,width=4pt]}, black, thin] (0,0) -- (135:1.5);
  \node[font=\scriptsize, black, above right=0pt] at (135:0.75) {$r$};
\end{tikzpicture}
\captionof{figure}{Sett ovenfra: slegge i en sleggekasters hånd. Snorkraften $\vec{S}$ fra wiren er alltid rettet inn mot sentrum.}
...
\exm{Slegge}{%
En sleggekaster svinger en slegge med masse $m = \SI{4.0}{\kilo\gram}$ rundt seg. Avstanden fra hånda til kula er $r = \SI{1.2}{\metre}$, og kula holder konstant fart $v = \SI{15}{\metre\per\second}$. Vi ser bort fra tyngdekraften.

Finn kraften i wiren.
}

\svar{%
Snorkraften $S$ fra wiren er den eneste kraften vi tar med, og den peker alltid inn mot sentrum (hånda). Den sørger for sentripetalakselerasjonen:
\[
    \sum F = \frac{mv^2}{r}
    \quad\implies\quad
    S = \frac{mv^2}{r}
    = \frac{\SI{4.0}{\kilo\gram} \cdot (\SI{15}{\metre\per\second})^2}{\SI{1.2}{\metre}}
    = \frac{\SI{900}{\newton\metre}}{\SI{1.2}{\metre}}
    = \doubleunderline{\SI{750}{\newton}}
\]
Det tilsvarer omtrent tyngden av en person på \SI{75}{\kilo\gram}, så sleggekasteren må holde godt fast!
}

\oppgaver{Karusell}{k4oppg11}{%
\input{Oppgaver/k4_oppg11.tex}%
}

% ---- Eksempel: Vogn i loop på tivoli ----
\noindent
\begin{minipage}[t]{0.54\linewidth}
\vspace{0pt}
I en loop på tivoli beveger vogna seg i en \emph{vertikal} sirkel. Da virker tyngdekraften delvis langs bevegelsesretningen, slik at farten endrer seg underveis: vogna går fortere i bunnen enn på toppen. Likevel gjelder sammenhengen for sentripetalkraft i hvert eneste punkt: kraftsummen inn mot sentrum må være $mv^2/r$, der $v$ er farten akkurat der vogna er.
\end{minipage}%
\hfill
\begin{minipage}[t]{0.43\linewidth}
\vspace{0pt}
...
\captionof{figure}{Vogn i en vertikal loop på tivoli, sett fra siden. Farten $v$ er tangentiell til sirkelen i alle fire posisjonene.}
\label{fig:loop_tivoli}
\end{minipage}

\medskip

\exm{Vogn i loop på tivoli}{%
...
\begin{enumerate}[a)]
    ...
    \item Hvor stor må $v_\text{C}$ være for at vogna ikke skal falle av på toppen?
\end{enumerate}
}

\svar{%
\noindent
\textbf{a)} To krefter virker på vogna: tyngdekraften $G = m\mathrm{g}$ (alltid rett ned) og normalkraften $\vec{N}$ fra skinnene. Vogna kjører på innsiden av loopen, så $\vec{N}$ peker alltid inn mot sentrum. Vi velger positiv retning \emph{inn mot sentrum} og bruker at $\sum F = mv^2/r$ i hver posisjon.

\smallskip\noindent
\textbf{A (bunnen):} $\vec{N}$ peker opp (inn mot sentrum), $\vec{G}$ peker ned (ut fra sentrum):
\[
    N_\text{A} - m\mathrm{g} = m\frac{v_\text{A}^2}{r}
    \quad\implies\quad
    N_\text{A} = m\frac{v_\text{A}^2}{r} + m\mathrm{g}
\]

\noindent
\textbf{B og D (halvveis opp og halvveis ned):} $\vec{N}$ er vannrett og peker inn mot sentrum. $\vec{G}$ står vinkelrett på denne retningen og bidrar ikke til kraftsummen inn mot sentrum (den endrer bare hvor fort vogna går):
\[
    N_\text{B} = m\frac{v_\text{B}^2}{r},
    \qquad
    N_\text{D} = m\frac{v_\text{D}^2}{r}
\]

\noindent
\textbf{C (toppen):} Både $\vec{N}$ og $\vec{G}$ peker ned, altså inn mot sentrum:
\[
    N_\text{C} + m\mathrm{g} = m\frac{v_\text{C}^2}{r}
    \quad\implies\quad
    N_\text{C} = m\frac{v_\text{C}^2}{r} - m\mathrm{g}
\]

\noindent
\textbf{b)} Skinnene kan bare \emph{skyve} på vogna, ikke trekke i den. Normalkraften kan derfor ikke bli negativ. Hvis vi får $N_\text{C} < 0$ i uttrykket over, betyr det at vogna ikke klarer å holde kontakten med skinnene --- den faller av. Kravet er altså
\[
    N_\text{C} \geq 0
    \quad\implies\quad
    m\frac{v_\text{C}^2}{r} - m\mathrm{g} \geq 0
    \quad\implies\quad
    \doubleunderline{v_\text{C} \geq \sqrt{\mathrm{g}\,r}}
\]
Massen faller ut: minstefarten på toppen er den samme for en tom og en full vogn.
}
...
\input{Oppgaver/k4_oppg12.tex}%
}

\oppgaver{Bilkollisjon}{k4oppg13}{%
\input{Oppgaver/k4_oppg13.tex}%
}
