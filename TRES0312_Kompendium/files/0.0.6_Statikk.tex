...
\begin{align*}
    ...
    \sum F_y = 0&:\quad S\sin\alpha + S\sin\alpha - G = 0 \implies 2S\sin\alpha = m\mathrm{g}
\end{align*}
...
\begin{tikzpicture}[scale=1.1, >={Stealth[length=6pt,width=4pt]}]
  ...
  \draw[->, line width=2.5pt, black!65]
      (\xMid, 0.28) -- (\xMid, 1.3) node[above, font=\small] {$M\mathrm{g}$};
...
\begin{itemize}
  \item Massesenter: $d_{M\mathrm{g}} = 2.0 - 0.5 = \SI{1.5}{\metre}$
  \item Søyle B: $d_B = (4.0 - 1.0) - 0.5 = \SI{2.5}{\metre}$
\end{itemize}

\textbf{Kraftligning ($\sum F_y = 0$):}
\[
  N_A + N_B = M\mathrm{g} = \SI{20}{\kilo\gram}\cdot\SI{9.81}{\metre\per\second\squared} = \SI{196.2}{\newton}
\]

\textbf{Momentligning om A ($\sum\tau_A = 0$, positivt mot klokken):}
\[
  N_B \cdot d_B - M\mathrm{g} \cdot d_{M\mathrm{g}} = 0
  \implies N_B = \frac{M\mathrm{g}\cdot d_{M\mathrm{g}}}{d_B}
...
\[
  N_A = M\mathrm{g} - N_B = 196.2 - 117.7 = \doubleunderline{\SI{78.5}{\newton}}
\]
}
...
\begin{tikzpicture}[scale=1.1, >={Stealth[length=6pt,width=5pt]}]
  ...
  % Vekt på enden
  \draw[->, line width=2.5pt, black!65]
      (3.0, -0.05) -- (3.0, -0.90) node[right, font=\small] {$m\mathrm{g}$};
...
Tyngdekraften $m\mathrm{g}$ lager et negativt (medurs) moment, den vertikale komponenten av stagkraften $T_y = T\sin\beta$ lager et positivt moment:
\[
  T\sin\beta\cdot L - m\mathrm{g}\cdot L = 0
  \implies T = \frac{m\mathrm{g}}{\sin\beta}
...
\begin{align*}
  ...
  \sum F_y = 0&:\quad H_y + T\sin\beta - m\mathrm{g} = 0 \implies H_y = m\mathrm{g} - T\sin\beta \approx \SI{0}{\newton}
\end{align*}
(Som forventet: momentligningen om hengselet sa nettopp at $T\sin\beta = m\mathrm{g}$, så $H_y = 0$.)
}
...
\svar{%
\textbf{Krefter:} normalkraft fra vegg $N_v$ (horisontal), normalkraft fra gulv $N_g$ (vertikal), friksjon fra gulv $f$ (horisontal), tyngdekraft på stige $m\mathrm{g}$ (ned fra midten), tyngdekraft på person $M\mathrm{g}$ (ned fra \SI{2.5}{\metre} langs stigen).

\textbf{Momentligning om bunnen:} (positiv mot klokken)
\begin{align*}
  &N_v\cdot L\sin\theta
  - m\mathrm{g}\cdot\tfrac{L}{2}\cos\theta
  - M\mathrm{g}\cdot d\cos\theta = 0
\end{align*}
der $d = \SI{2.5}{\metre}$. Løser for $N_v$:
\[
  N_v = \frac{(m/2 + Md/L)\,\mathrm{g}\cos\theta}{\sin\theta}
...
\textbf{Kraftligninger:}
\[
  N_g = (m+M)\mathrm{g} = \SI{85}{\kilo\gram}\cdot\SI{9.81}{\metre\per\second\squared}
...
\begin{tikzpicture}[scale=1.0, >={Stealth[length=6pt,width=4pt]}]
  ...
  \draw[->, line width=1.8pt, black!65] (-1.5,0) -- (-1.5,-0.80) node[below, font=\footnotesize] {$m_1 \mathrm{g}$};
  ...
  \draw[->, line width=1.8pt, black!65] (1.0,0) -- (1.0,-0.80) node[below, font=\footnotesize] {$m_2 \mathrm{g}$};
...
\[
  x_2 = -\frac{m_1}{m_2}\,x_1
      ...
      = +\SI{0.857}{\metre} \approx \doubleunderline{\SI{0.86}{\metre}}
\]
