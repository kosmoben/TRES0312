...
\node[anchor=south west] (thumb) at (0,0) {\includegraphics[width=3.6cm]{Images/YouTubeLenker/YT_FartIFysikk.png}};
\node[anchor=north west] at (thumb.south west) {\includegraphics[width=1.2cm]{Images/YouTubeLenker/Icon.png}};
\end{tikzpicture}%
}}%
\end{wrapfigure}
...
\begin{tikzpicture}[inner sep=0pt]
\node[anchor=south west] (thumb) at (0,0) {\includegraphics[width=3.6cm]{Images/YouTubeLenker/YT_PosisjonsOgFartsgraf.png}};
\node[anchor=north west] at (thumb.south west) {\includegraphics[width=1.2cm]{Images/YouTubeLenker/Icon.png}};
\end{tikzpicture}%
}}%
\end{wrapfigure}
...
I krumlinjet bevegelse er ikke gjennomsnittsfart lenger et like nyttig begrep, dersom vi forstår det som størrelsen på hastighetsvektoren mellom to punkt. Størrelsen på den totale forflytningen dividert med den totale tiden gir nemlig ikke nødvendigvis et riktig inntrykk av hvor fort et legeme faktisk har beveget seg (med mindre tidsintervallet er veldig kort). For eksempel, hvis et objekt beveger seg i en sirkel og returnerer til utgangspunktet, vil den totale forflytningsvektoren være null (prøv å tegne opp $\vec{\Delta s}$ mellom start og slutt!) --- og dermed er både gjennomsnittshastigheten \emph{og} gjennomsnittsfarten null, om vi måler over én runde. Men jeg er ganske sikker på at Karsten Warholm har løpt veldig fort rundt friidrettsbanen selv om gjennomsnittsfarten hans er null, målt over hver runde... Problemet oppstår fordi hastighet er definert ut ifra forflytningsvektoren $\vec{\Delta s}$. Dermed får også farten, som er størrelsen på hastighetsvektoren, det samme problemet. Det er derfor nyttig å innføre et begrep til, kalt \textit{banefart}. Dette er farten som et legeme har til enhver tid langs den buen, eller banen, det beveger seg. 
...
\textbf{e)} Hva er gjennomsnittlig banefart til båt 2 på hele strekningen?
}
...
\begin{enumerate}[\textbf{\alph*)}]
    ...
    \item Gjennomsnittsfart per etappe for båt 2 er lengden av de tilhørende hastighetsvektorene fra b):
          ...
          Selv om båt 2 hadde ulik fart på de to etappene, er den totale gjennomsnittsfarten altså identisk med båt 1. Gjennomsnittsfart (forstått som lengden på hastighetsvektoren) avhenger kun av total forflytning og total tid --- ikke av hvordan hastighetsvektoren kan ha endret størrelse og retning underveis. Vi ser at det er ikke denne farten vi er interessert i. Det egentlig interessante fartsbegrepet i akkurat denne sammenheng, er banefarten, som vi ser på i neste deloppgave.
...
Fra eksemplet over, med båtene, ser vi at når vi ikke lenger har en rettlinjet bevegelse, så er det nødvendig med en presisering. \textbf{Vi må skille mellom total forflytning og strekning}. Forflytning er en vektor som beskriver endring i posisjon, mens strekning er en skalar som beskriver hvor langt et objekt faktisk har beveget seg, om vi summerer forflytninger langs banen.
...
\begin{enumerate}[\textbf{\alph*)}]
  ...
  \item Farten $|\vec{v}|$ til Karsten er konstant hele veien, men siden banen er krum, er strekningen han faktisk løper lengre enn forflytningen $|\vec{\Delta s}_{PQ}|$ fra a). Buelengden langs en kvart sirkel er:
        ...
        Banefarten på $\SI{8.1}{\metre\per\second}$ er altså høyere enn gjennomsnittsfarten på $\SI{7.3}{\metre\per\second}$ fra a) --- nettopp fordi gjennomsnittsfarten kun ser på nettoforflytningen $PQ$, og ikke fanger opp at Karsten faktisk løper en lengre, krum bane. Gjennomsnittsfarten, forstått som lengden av hastighetsvektoren,er derfor et dårlig mål på farten hans når tidsintervallet vi tar gjennomsnittet over, blir for stort.
\end{enumerate}
}
\merk{Hva mener vi med \enquote{gjennomsnittsfart}?}{%
Begrepet \enquote{gjennomsnittsfart} er altså et generisk, litt utydelig begrep. Ut ifra sammenhengen må vi forstå om det er snakk om størrelsen til hastighetsvektoren, eller banefarten.
}
...
Fra eksemplet med de to båtene, samt Karsten Warholm, så ser vi at hva gjennomsnittsfarten er (forstått som lengden av hastighetsvektoren) er veldig avhengig av hvor ofte vi måler. Desto kortere tidsintervall vi måler på, desto mer interessant blir resultatet. I praksis kan vi ofte måle posisjonen til et legeme veldig ofte, og dermed kan vi finne ut hva farten er på et gitt tidspunkt. Dette kaller vi for \textit{momentanhastighet}. Lengden av momentanhastigheten kaller vi \textit{momentanfart}. Dette er et veldefinert og nyttig begrep.
...
\subsubsection{Konstant fart}
 Vi kan tilslutt merke oss at desom farten ikke endrer seg, men er konstant, da er \textit{momentanfarten} til enhver tid den samme som \textit{gjennomsnittlig banefart}. 

\oppgaver{Konstant hastighet}{k3oppg3}{%
\input{Oppgaver/k3_oppg03.tex}%
}

\oppgaver{En oppgave til}{k3oppg4}{%
\input{Oppgaver/k3_oppg04.tex}%
}
...
\begin{tikzpicture}[inner sep=0pt]
\node[anchor=south west] (thumb) at (0,0) {\includegraphics[width=3.6cm]{Images/YouTubeLenker/YT_Akselerasjon.png}};
\node[anchor=north west] at (thumb.south west) {\includegraphics[width=1.2cm]{Images/YouTubeLenker/Icon.png}};
\end{tikzpicture}%
}}%
\end{wrapfigure}
...
\begin{tikzpicture}[inner sep=0pt]
\node[anchor=south west] (thumb) at (0,0) {\includegraphics[width=3.6cm]{Images/YouTubeLenker/YT_KonstantAkselerasjon.png}};
\node[anchor=north west] at (thumb.south west) {\includegraphics[width=1.2cm]{Images/YouTubeLenker/Icon.png}};
\end{tikzpicture}%
}}%
\end{wrapfigure}
...
\begin{wrapfigure}[8]{r}[0.3cm]{2.2cm}
\vspace{-\baselineskip}%
\rotatebox{-15}{\href{https://youtu.be/WxmJxCtOBOM?si=Yga6Pbs2UgeSTidM}{%
\begin{tikzpicture}[inner sep=0pt]
\node[anchor=south west] (thumb) at (0,0) {\includegraphics[width=2.2cm]{Images/YouTubeLenker/YT_MaleHoydeMedStein.png}};
\node[anchor=north west] at (thumb.south west) {\includegraphics[width=1.2cm]{Images/YouTubeLenker/Icon.png}};
\end{tikzpicture}%
}}%
\end{wrapfigure}
Her på jorden blir vi påvirket av en tyngdekraft, som gjør at alt blir dratt mot senter av jorden. Det som kanskje er litt overraskende, er at alle legemer vil ha samme akselerasjon ned mot bakken (når vi ser bort ifra luftmotstand). Denne akselerasjonen er konstant og kalles $\mathrm{g}$. Størrelsen er
{\setlength{\linewidth}{9cm}\graabox{$\displaystyle \text{Tyngdens akselerasjon:}\quad\mathrm{g} = \SI{9.81}{\metre\per\second\squared}$}}

I videoen slippes en stein fra en bro for å måle brohøyden --- akkurat dette scenarioet går vi gjennom numerisk (med og uten luftmotstand) i notatboken det er lenket til under.
\pyfile{TRES0312 Python/Kap3\_Kinematikk/K3\_steinkast\_luftmotstand.ipynb}{Kap3_Kinematikk/K3_steinkast_luftmotstand.ipynb}
...
\input{Oppgaver/k3_oppg06.tex}%
}

\oppgaver{Fritt fall}{k3oppg7}{%
\input{Oppgaver/k3_oppg07.tex}%
}
...
\paragraph{Dekomponering.} Se for deg en stein som kastes utfor en klippe. Steinen vil falle nedover på grunn av tyngdekraftens konstante akselerasjon på $\mathrm{g}=\SI{9.81}{\metre\per\second\squared}$. Men i horisontal retning så vil steinen bare fortsette rett fram uten å akselerere. Vi må \textit{dekomponere} likningene og analysere hver retning for seg. 
...
\tips{Fortegn ved dekomponering}{Husk at når du dekomponerer så er alle fortegn avhengig av hva du har valt som positiv retning for hver koordinat-akse. For eksempel, hvis du velger oppover som positiv y-retning, så vil tyngdekraftens akselerasjon i y-retning være negativ, altså $a_y = -\mathrm{g}$. Hvis du velger nedover som positiv y-retning, så vil tyngdekraftens akselerasjon i y-retning være positiv, altså $a_y = \mathrm{g}$. Dette er bare et valg av koordinatsystem, og har ingen innvirkning på fysikken.
}
...
Håland skal skyte et straffespark, og skyter ballen med en utgangsvinkel på $18^\circ$ i forhold til bakken, se figur~\ref{fig:straffespark}. Straffemerket ligger $\SI{11}{\metre}$ fra mållinjen. Et fotballmål er internasjonalt standardisert til å være $\SI{2.44}{\metre}$ høyt (avstanden fra bakken til undersiden av tverrliggeren). Ballen har imidlertid selv en viss utstrekning, så det holder at \textit{senteret} av ballen befinner seg i høyden $\SI{2.30}{\metre}$ for at ballen så vidt skal passere under tverrliggeren. Håland ønsker å skyte ballen slik at den passerer rett under tverrliggeren, akkurat idet den når mållinjen. Vi ser bort fra luftmotstand. Hvor stor må utgangsfarten $v_0$ være?\\
   ...
   \begin{center}
        \usetikzlibrary{calc}
    \begin{tikzpicture}
    \node[anchor=south west, inner sep=0pt] (img) at (0,0) {\includegraphics[width=0.78\linewidth]{Images/Kapittel2:Kinematikk/Straffespark_Haaland.png}};
    \newcommand{\imgpt}[3]{%
    ...
    \end{tikzpicture}
    \captionof{figure}{Straffespark. Ballens bane er vist med stiplet linje, med utgangsfarten $\vec{v}_0$ tegnet inn som tangent til banen ved avspark.}
    ...
    \textit{Metode:}
    Ballens bane er krumlinjet, så vi dekomponerer bevegelsen i en horisontal ($x$) og en vertikal ($y$) retning, og analyserer hver retning for seg. Vi lar $x$ være den horisontale avstanden fra straffemerket, og $y$ være høyden over bakken, slik at posisjonsvektoren er $\vec{s}(t) = [x(t), y(t)]$. Ballen starter i origo, altså $\vec{s}_0 = [x_0, y_0] = [0, 0]$. Utgangsfarten $v_0$ peker i vinkelen $\theta = 18^\circ$ over bakken, og dekomponeres som
    \[
    \vec{v}_0 = [v_{0x}, v_{0y}] = [v_0\cos\theta,\ v_0\sin\theta].
    \]
    Ballen påvirkes kun av tyngdekraften (vi ser bort fra luftmotstand), så det er ingen akselerasjon i $x$-retning, mens akselerasjonen i $y$-retning er tyngdeakselerasjonen. Akselerasjonsvektoren er dermed
    \[
    \vec{a} = [a_x, a_y] = [0,\ -\mathrm{g}].
    \]
    Den generelle (vektorielle) posisjonslikningen for konstant akselerasjon er
    \[
    \vec{s}(t) = \vec{s}_0 + \vec{v}_0\,t + \tfrac{1}{2}\vec{a}\,t^2.
    \]
    Dette er en vektorlikning, og den gjelder derfor komponent for komponent. I $x$-retning gir den
    \[
    x(t) = x_0 + v_{0x}\,t + \tfrac{1}{2}a_x\,t^2,
    \]
    og setter vi inn $x_0 = 0$, $v_{0x} = v_0\cos\theta$ og $a_x = 0$ fra vektorene over, får vi
    \[
    x(t) = v_0 \cos\theta \cdot t.
    \]
    På samme måte, i $y$-retning:
    \[
    y(t) = y_0 + v_{0y}\,t + \tfrac{1}{2}a_y\,t^2,
    \]
    og setter vi inn $y_0 = 0$, $v_{0y} = v_0\sin\theta$ og $a_y = -\mathrm{g}$, får vi
    \[
    y(t) = v_0 \sin\theta \cdot t - \frac{1}{2} \mathrm{g} t^2.
    \]
    Posisjonslikningene blir altså:
    \[
    x(t) = v_0 \cos\theta \cdot t, \qquad y(t) = v_0 \sin\theta \cdot t - \frac{1}{2} \mathrm{g} t^2.
    \]
    ...
    \[
    2.30 = v_0 \sin\theta \cdot \frac{11}{v_0 \cos\theta} - \frac{1}{2} \mathrm{g} \left( \frac{11}{v_0 \cos\theta} \right)^2 = 11 \tan\theta - \frac{121\, \mathrm{g}}{2 v_0^2 \cos^2\theta}.
    \]
    Legg merke til at $v_0$ forsvinner fra det første leddet, siden $\sin\theta / \cos\theta = \tan\theta$. Vi løser likningen for $v_0^2$:
    \[
    v_0^2 = \frac{121\, \mathrm{g}}{2\cos^2\theta \left( 11\tan\theta - 2.30 \right)}.
    \]
    Setter vi inn tallverdiene $\mathrm{g} = \SI{9.81}{\metre\per\second\squared}$ og $\theta = 18^\circ$, får vi
    \[
...
\end{itemize}
    ...
    \begin{center}
    \includegraphics[width=0.55\linewidth]{Images/Kapittel2:Kinematikk/RallybilKlippe.png}
    \captionof{figure}{Rallybilen kjører utfor klippen}
    \label{fig:rallybil}
    \end{center}
    ...
    \textit{Metode:}
    Idet bilen kjører utfor klippen blir bevegelsen krumlinjet, og vi dekomponerer den igjen i en horisontal ($x$) og en vertikal ($y$) retning, med posisjonsvektor $\vec{s}(t) = [x(t), y(t)]$. I dette øyeblikket har bilen kun horisontal fart (den faller ikke ennå), så utgangsfarten dekomponeres som
    \[
    \vec{v}_0 = [v_{0x}, v_{0y}] = [v_1,\ 0] = [\SI{8.33}{\metre\per\second},\ 0],
    \]
    og akselerasjonen er igjen kun tyngdeakselerasjonen, $\vec{a} = [a_x, a_y] = [0,\ -\mathrm{g}]$. Vi lar $x_0 = 0$ ved klippekanten, og $y_0 = \SI{5}{\metre}$ være klippens høyde over bakken der bilen lander ($y=0$). Fra den generelle vektorlikningen $\vec{s}(t) = \vec{s}_0 + \vec{v}_0\,t + \tfrac{1}{2}\vec{a}\,t^2$ får vi, komponent for komponent:
    \[
    x(t) = x_0 + v_{0x}\,t + \tfrac{1}{2}a_x t^2 = v_1 t,
    \qquad
    y(t) = y_0 + v_{0y}\,t + \tfrac{1}{2}a_y t^2 = y_0 - \tfrac{1}{2} \mathrm{g} t^2,
    \]
    der vi har satt inn $x_0 = 0$, $a_x = 0$ og $v_{0y} = 0$ fra vektorene over. Bilen treffer bakken når $y(t) = 0$, som gir tiden $t$ den er i lufta:
    \[
    0 = y_0 - \frac{1}{2} \mathrm{g} t^2 \quad \Rightarrow \quad 0 = 5 - \frac{1}{2}(9.81) t^2 \quad \Rightarrow \quad t = \sqrt{\frac{2\cdot 5}{9.81}} \approx \SI{1.01}{\second}.
    \]
    Horisontal avstand følger da av $x(t) = v_1 t$:
    \[
    ...
    \textit{Metode:}
    Hastighetsvektoren følger på samme måte fra den generelle vektorlikningen $\vec{v}(t) = \vec{v}_0 + \vec{a}\,t$, som komponent for komponent gir
    \[
    v_x(t) = v_{0x} + a_x t, \qquad v_y(t) = v_{0y} + a_y t.
    \]
    Setter vi inn $v_{0x} = 8.33\,\si{\metre\per\second}$, $a_x = 0$, $v_{0y}=0$ og $a_y=-\mathrm{g}$ fra vektorene i Del b, ser vi at den horisontale hastighetskomponenten er uendret, $v_x = v_{0x} = \SI{8.33}{\metre\per\second}$, mens den vertikale komponenten blir
    \[
    v_y = -\mathrm{g} t = -9.81 \cdot 1.01 \approx \SI{-9.90}{\metre\per\second}.
    \]
...
\input{Oppgaver/k3_oppg08.tex}%
}

\oppgaver{Prosjektilbevegelse}{k3oppg9}{%
\input{Oppgaver/k3_oppg09.tex}%
}
...
\begin{tikzpicture}[inner sep=0pt]
\node[anchor=south west] (thumb) at (0,0) {\includegraphics[width=3.6cm]{Images/YouTubeLenker/YT_Sentripetalakselerasjon.png}};
\node[anchor=north west] at (thumb.south west) {\includegraphics[width=1.2cm]{Images/YouTubeLenker/Icon.png}};
\end{tikzpicture}%
}}%
\end{wrapfigure}
...
\]
Dette er omtrent $6.1\cdot10^{-3}$ ganger tyngdeakselerasjonen $\mathrm{g}$. Selv om akselerasjonen er svært liten, er det nok til å holde jorda i bane rundt sola — fordi tyngdekraften virker over en enorm avstand.
}

\oppgaver{Sirkelbevegelse og omløpstid}{k3oppg11}{%
\input{Oppgaver/k3_oppg11.tex}%
}
