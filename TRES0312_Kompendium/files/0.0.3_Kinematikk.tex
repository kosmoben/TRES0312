...
{\setlength{\linewidth}{9cm}\graabox{$\displaystyle \text{Tyngdens akselerasjon:}\quad\text{g} = \SI{9.81}{\metre\per\second\squared}$}}
...
Håland skal skyte et straffespark, og skyter ballen med en utgangsvinkel på $18^\circ$ i forhold til bakken, se figur~\ref{fig:straffespark}. Straffemerket ligger $\SI{11}{\metre}$ fra mållinjen. Et fotballmål er internasjonalt standardisert til å være $\SI{2.44}{\metre}$ høyt (avstanden fra bakken til undersiden av tverrliggeren). Ballen har imidlertid selv en viss utstrekning, så det holder at \textit{senteret} av ballen befinner seg i høyden $\SI{2.30}{\metre}$ for at ballen så vidt skal passere under tverrliggeren. Håland ønsker å skyte ballen slik at den passerer rett under tverrliggeren, akkurat idet den når mållinjen. Vi ser bort fra luftmotstand. Hvor stor må utgangsfarten $v_0$ være?\\
   ...
   \begin{center}
    ...
    \]
    og setter vi inn $y_0 = 0$, $v_{0y} = v_0\sin\theta$ og $a_y = -g$, får vi
    \[
...
\end{itemize}
    ...
    \]
    og akselerasjonen er igjen kun tyngdeakselerasjonen, $\vec{a} = [a_x, a_y] = [0,\ -g]$. Vi lar $x_0 = 0$ ved klippekanten, og $y_0 = \SI{5}{\metre}$ være klippens høyde over bakken der bilen lander ($y=0$). Fra den generelle vektorlikningen $\vec{s}(t) = \vec{s}_0 + \vec{v}_0\,t + \tfrac{1}{2}\vec{a}\,t^2$ får vi, komponent for komponent:
    \[
    x(t) = x_0 + v_{0x}\,t + \tfrac{1}{2}a_x t^2 = v_1 t,
    \qquad
    y(t) = y_0 + v_{0y}\,t + \tfrac{1}{2}a_y t^2 = y_0 - \tfrac{1}{2} g t^2,
    \]
    ...
    \[
    0 = y_0 - \frac{1}{2} g t^2 \quad \Rightarrow \quad 0 = 5 - \frac{1}{2}(9.81) t^2 \quad \Rightarrow \quad t = \sqrt{\frac{2\cdot 5}{9.81}} \approx \SI{1.01}{\second}.
    \]
    ...
    \]
    Setter vi inn $v_{0x} = 8.33\,\si{\metre\per\second}$, $a_x = 0$, $v_{0y}=0$ og $a_y=-g$ fra vektorene i Del b, ser vi at den horisontale hastighetskomponenten er uendret, $v_x = v_{0x} = \SI{8.33}{\metre\per\second}$, mens den vertikale komponenten blir
    \[
    v_y = -g t = -9.81 \cdot 1.01 \approx \SI{-9.90}{\metre\per\second}.
    \]
...
\begin{enumerate}[a)]
    ...
    \item Sammenlikn denne akselerasjonen med tyngdens akselerasjon $g$. Er den stor eller liten i forhold?
\end{enumerate}
}
