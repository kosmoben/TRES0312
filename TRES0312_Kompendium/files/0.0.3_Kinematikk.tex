...
\exm{Prosjektil og beregning av sluttposisjon}{
Et prosjektil skytes ut med en kjent utgangshastighet \( v_0 \), i en vinkel \( \theta \) over horisontalen. Vi ser bort fra luftmotstand. Målet er å finne hvor prosjektilet lander, altså sluttposisjonen. Vi skal først bruke posisjonslikningen i y-retning for å finne hvor lenge prosjektilet er i lufta, og deretter bruke tiden \( t \) til å finne sluttposisjonen i x-retning.\\

    \textbf{Del a: Beregn tiden prosjektilet er i lufta} \\

    \textit{Metode:}
    For å finne hvor lenge prosjektilet er i lufta, bruker vi posisjonslikningen i y-retning. Siden prosjektilet returnerer til utgangshøyden (y-posisjon = 0) på slutten av sin bevegelse, kan vi bruke følgende likning:
    \[
    y = y_0 + v_{0y} t + \frac{1}{2} a_y t^2.
    \]
    Vi vet at sluttposisjonen \( y = 0 \), startposisjonen \( y_0 = 0 \), og akselerasjonen \( a_y = -9.81 \, \si{\metre\per\second\squared} \) (tyngdeakselerasjonen, siden vi ser bort fra luftmotstand). Den vertikale komponenten av utgangshastigheten \( v_{0y} \) er gitt som \( v_{0y} = v_0 \sin \theta \). Dermed får vi:
    \[
    0 = v_0 \sin \theta \cdot t - \frac{1}{2} g t^2.
    \]
    Vi løser denne kvadratiske likningen for \( t \) (tiden prosjektilet er i lufta):
    \[
    t = \doubleunderline{\frac{2 v_0 \sin \theta}{g}}
    \]

    \textbf{Del b: Beregn sluttposisjonen i x-retning} \\

    \textit{Metode:}
    Nå som vi vet hvor lenge prosjektilet er i lufta, kan vi bruke tiden \( t \) til å beregne hvor langt prosjektilet beveger seg i x-retning. Vi bruker posisjonslikningen i x-retning:
    \[
    x = v_{0x} t + \frac{1}{2} a_x t^2.
    \]
    Siden det ikke er noen akselerasjon i x-retning (\( a_x = 0 \), fordi vi ser bort fra luftmotstand), blir likningen:
    \[
    x = v_{0x} t,
    \]
    ...
    \[
    x = v_0 \cos \theta \cdot \frac{2 v_0 \sin \theta}{g} = \doubleunderline{\frac{2 v_0^2 \sin \theta \cos \theta}{g}}.
    \]

    Siden sluttposisjonen i y-retning er \( y = 0 \) når prosjektilet lander, kan vi skrive sluttposisjonen som en vektor:
   \[
    [x, y] = \left[\frac{2 v_0^2 \sin \theta \cos \theta}{g},\ 0\right].
    \]
    Dermed er sluttposisjonen gitt som vektor, der x-komponenten er \( \dfrac{2 v_0^2 \sin \theta \cos \theta}{g} \) og y-komponenten er \( 0 \).
}
...
\]
\[
t = \sqrt{\frac{1.6}{9.81}} \approx \doubleunderline{0.4 \, \si{\second}}
\]
...
Løs for \(v_{0x}\):
\[
v_0 = v_{0x} = \frac{0.85}{0.4} \approx \doubleunderline{2.125 \, \si{\metre\per\second}}
\]
...
Dermed er sluttfarten rett før ballen treffer bakken \(v_1 \approx \doubleunderline{4.46 \, \si{\metre\per\second}}\).
}
...
\oppgave{Straffespark}{%
Håland skal skyte et straffespark, og skyter ballen med en utgangsvinkel på $18^\circ$ i forhold til bakken. Straffemerket ligger $\SI{11}{\metre}$ fra mållinjen. Et fotballmål er internasjonalt standardisert til å være $\SI{2.44}{\metre}$ høyt (avstanden fra bakken til undersiden av tverrliggeren). Håland ønsker å skyte ballen slik at den passerer rett under tverrliggeren, akkurat idet den når mållinjen. Se bort ifra luftmotstand.
\begin{enumerate}[a)]
    \item Tegn en ryddig figur med relevante størrelser innmerket.
    \item Sett opp uttrykk for posisjonen til ballen i $x$- og $y$-retning som funksjon av tiden $t$.
    \item Bruk uttrykket for $x(t)$ til å finne et uttrykk for tiden $t^\ast$ det tar før ballen når mållinjen (uttrykt ved utgangsfarten $v_0$).

    \textit{Hint:} Bruk at $x(t^\ast) = \SI{11}{\metre}$, og at $v_{0x} = v_0\cos 18^\circ$.

    \item Sett uttrykket for $t^\ast$ inn i likningen for $y(t)$, og bruk at $y(t^\ast) = \SI{2.44}{\metre}$ til å finne utgangsfarten $v_0$.

    \textit{Hint:} Dette gir én likning med kun $v_0$ som ukjent --- løs den med hensyn på $v_0$.
\end{enumerate}
}
