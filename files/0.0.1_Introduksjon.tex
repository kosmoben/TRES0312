...
\intro{Verden består av så mye forskjellig. Her har vi natur, med skoger og fjell, elver og vann. Her lever alle mulige biologiske vesener. Her finner vi også mennesket, som bygger hus, skyskrapere og teknologiske dupeditter som mobiltelefoner. Der oppe på kuleskallet ser vi stjerner og kométer, supernovaer og nordlys. Felles for alle disse svært forskjellige delene av verden er at det lar seg beskrive i matematikk. Kan alt i verden beskrives i matematikk? Hva med \textit{kjærlighet} eller \textit{bevissthet}? Slike vanskelige, filosofiske spørsmål skal vi ikke vsare på i dette kompendiet, men de er verdt å merke seg. 

\paragraph{I dette kapitlet} skal vi se på diverse matematiske verktøy som blir brukt i kompendiet. Jeg anbefaler å lese det når du får bruk for det, og ikke nødvendigvis i sin helhet.}

\spm{}{%
Hva er fysikk?\\
Hvordan beskriver vi virkeligheten med matematikk?}
\newpage
...
\defn{Fysisk størrelse}{En fysisk størrelse har både en verdi og en enhet:
\begin{align}
    &\textrm{Størrelse} = \textrm{verdi} \cdot \textrm{enhet} \\ 
    &L=50\cdot \textrm{kilometer}
\end{align}}

Men... hvor mange forskjellige enheter trenger man for å beskrive \textit{alt} i naturen?

\subsection{SI-enheter}
Det viser seg at man ikke trenger flere enn 7 forskjellige enheter! Det finnes mange forskjellige systemer for enheter. Felles for dem er at de koker ned til 7 unike enheter. I dette faget skal vi benytte SI-enheter, som er det internasjonale enhetssystemet sitt system, og benyttet i mange land. Figur \ref{fig:SI_enheter} viser en illustrasjon av SI-enhetene, og i dette faget skal vi benytte alle, så nær som \textit{candela}, som er måleenhet for \textit{opplevd lysstyrke}. 
...
Tabell \ref{tab:SI_enheter} viser måleenheter fra SI-enhetene og hva de måler.
...
\end{table}
I 2018 vedtok Generalkonferansen for mål og vekt at SI-systemet ikke lenger skal definires ut ifra menneskeskapte prototyper, slik som gullstandarden for 1kg i Paris og den slags. I stedet skal enhetene definieres ut ifra fysiske konstanter. Dette gjøres ved at de fysiske konstantene får \textit{definerte} verdier, og så endrer man verdien til sekund, meter, kg, osv ut ifra disse definisjonene. En skjematisk oppsummering er gitt i Figur~\ref{fig:SI_enheter2}
...
\end{figure}
Den ytterste ringen i Figur~\ref{fig:SI_enheter2} representerer altså såkalte \textit{naturkonstanter}.Tabell~\ref{tab:natKonst} listes symbol, navn og verdi for disse opp. Vi kan spesielt merke oss de som er skrevet i blått, da disse er de mest sentrale i dette kurset.
...
\hline
\rowcolor{blue!20!black!10}
\textbf{Symbol} & \textbf{Navn} & \textbf{Verdi (eksakt)} \\
\hline
$c$ & Lyshastigheten i vakuum & $299\,792\,458 \ \text{m}/\textrm{s}$ \\
$h$ & Plancks konstant & $6.626\,070\,15 \cdot 10^{-34} \ \text{J s}$ \\
$e$ & {\color{blue}Elementærladning} & $1.602\,176\,634 \cdot 10^{-19} \ \text{C}$ \\
$N_A$ & {\color{blue}Avogadros konstant} & $6.022\,140\,76 \cdot 10^{23} \ \text{mol}^{-1}$ \\
$k_B$ & {\color{blue}Boltzmanns konstant} & $1.380\,649 \cdot 10^{-23} \ \text{J K}^{-1}$ \\
$\Delta \nu_{Cs}$ & Cesium-frekvens (hyperfin overgang) & $9\,192\,631\,770 \ \text{Hz}$ \\
$K_{cd}$ & Luminøs effektivitet & $683 \ \text{lm W}^{-1}$ \\
\hline
\end{tabular}
\caption{Naturkonstanter som definerer SI-systemet (2018-definisjonen).}
\label{tab:natKonst}
\end{table}
...
Man kan bruke algebra-regler for å regne med enheter.
\end{myoppsum*}
...
\begin{align}
\hat{x} = (1,0), \qquad \hat{y} = (0,1)
\end{align}

Her peker $\hat{x}$ i positiv $x$-retning, mens $\hat{y}$ peker i positiv $y$-retning. Sammen danner disse enhetsvektorene grunnlaget for koordinatsystemet.

\begin{mymerk*}{Alternativ notasjon for enhetsvektorer}
I mange lærebøker brukes symbolene $i$, $j$, $k$ for de samme enhetsvektorene, og noen steder skrives disse med hat som $\hat{i}$, $\hat{j}$, $\hat{k}$. I dette kompendiet bruker vi $\hat{x}$, $\hat{y}$ (og $\hat{z}$ i tre dimensjoner), da denne notasjonen gjør det umiddelbart tydelig hvilken retning enhetsvektoren peker i.
\end{mymerk*}
...
\begin{enumerate}[a)]
    \item Skriv vektoren $\vec{A} = (4,\,-3)$ som en lineær kombinasjon av enhetsvektorene $\hat{x}$ og $\hat{y}$.
    \item Regn ut summen $\vec{u} + \vec{v}$ der $\vec{u} = 3\hat{x} - 2\hat{y}$ og $\vec{v} = -\hat{x} + 5\hat{y}$. Skriv svaret på komponentform.
\end{enumerate}
}
...
\begin{align*}
    \vec{\mathrm{B}} &= (3,0) = 3\hat{x} + 0 \hat{y}\\
    \vec{\mathrm{C}} &= (0,2) = 0 \hat{x} + 2 \hat{y}\\
    \vec{\mathrm{A}} &= (3+0)\hat{x} + (0+2)\hat{y} \\
    &= 3 \hat{x} + 2 \hat{y} = (3,2)
\end{align*}
...
Skalarproduktet er spesielt nyttig for å
\begin{itemize}
    \item finne vinkelen mellom to vektorer
    \item beregne arbeid ($W = \vec{F}\cdot\vec{s}$)
    \item sjekke om to vektorer er vinkelrette: $\vec{u}\cdot\vec{v}=0$ dersom og bare dersom $\theta=90^\circ$
\end{itemize}
